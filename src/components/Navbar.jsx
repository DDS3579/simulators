...
const simulatorCategories = [
  {
    ...
    items: [
      ...
      { name: "Newton's Laws", path: '/simulators/newtons-laws', icon: '🍎', ready: true },
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-18">
          ...
          <Link to="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              <span className={`text-lg font-extrabold tracking-tight transition-colors duration-300 ${scrolled ? 'text-gray-900' : 'text-white'
                }`}>
                SciSim
              </span>
              <span className={`text-lg font-extrabold tracking-tight transition-colors duration-300 ${scrolled ? 'text-indigo-600' : 'text-indigo-300'
                }`}>
                Lab
              </span>
              <div className={`text-[9px] font-medium -mt-1 transition-colors duration-300 ${scrolled ? 'text-gray-400' : 'text-white/60'
                }`}>
                Nepal Curriculum
              </div>
            </div>
            ...
            {simulatorCategories.map((category) => (
              ...
              >
                ...
                >
                  ...
                  {category.name}
                  <ChevronDown size={14} className={`transition-transform duration-300 ${activeDropdown === category.name ? 'rotate-180' : ''
                    }`} />
                </button>
                ...
                <div
                  className={`absolute top-full left-0 mt-1 w-64 bg-white rounded-xl shadow-2xl 
                            ...
                            transition-all duration-300 origin-top ${activeDropdown === category.name
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden transition-all duration-500 overflow-hidden ${isOpen ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0'
          }`}
      >
